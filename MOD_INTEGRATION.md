...
### 3. Получение вайтлиста сервера (для модов на сервере)

**GET** `/game-servers/{server_id}/whitelist`

**Заголовок:** `X-Ingest-Token: <ingest_token>` — обязателен (токен настраивается на бэкенде).

**Ответ:**
```json
{
  "nicknames": ["Player1", "Nickname2", ...]
}
```

Список одобренных ников для вайтлиста. Мод на игровом сервере должен запрашивать этот эндпоинт с ingest-токеном и использовать возвращённый список для проверки доступа.

### 4. Получение списка всех бэйджиков
